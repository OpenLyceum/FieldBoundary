...
1. Copies the `intro/` prototype into `src/<kebab>/` per screen
...
4. Removes the prototype `intro/` folder
...
main.ts
  └─ IntroScreen            (Screen<IntroModel, IntroScreenView>)
       ├─ IntroModel         owns all state
       └─ IntroScreenView    owns all visuals
```
...
template's `IntroScreen.ts`. Screen icons live in one shared module under
...
```
src/common/FieldBoundaryScreenIcons.ts
```
